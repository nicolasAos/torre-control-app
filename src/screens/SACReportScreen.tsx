...
class SACReportClass extends Component {
  ...
  componentDidMount(): void {
    Logger.log('mount SACReport screen');
  }

  componentWillUnmount(): void {
    Logger.log('unmount SACReport screen');
  }

  getNfs = () => {
    Logger.log('getNfs');
    getNfsSupervisorOffline(this.state.nf_id)
      .then(async (data) => {
        this.setState({
          data: data,
        });
  ...
  getAllNovedades = () => {
    Logger.log('getAllNovedades');
    this.setState({loading: true});
    getNovedades(this.props.navigation.state.params.sacMenu)
      .then((response) => {
    ...
    Geolocation.getCurrentPosition(
      (info) => {
        sendEmptyRemission(
          ...
          // sendEmptyRemission(this.props.login.user_id, data[0], '', '', info.coords.latitude, info.coords.longitude,
          ...
          .then(async (response) => {
            ...
            await sendEvidenceArr(data[0], newArr);
          ...
          ImageResizer.createResizedImage(
            ...
            this.setState({
              photos: [
                ...this.state.photos,
                {
                  name: `reportesac_${this.state.data[0].nf_id}_${this.state.photos.length}`,
                  uri: image.uri,
                  type: 'image/png',
      ...
      <View style={styles.containerHeader}>
        ...
        <View style={styles.containerHeaderItem}>
          ...
          <Text style={styles.headerItem}>
            2{/* {parseFloat(this.state.data.nf_volume)} */}
          </Text>
        </View>
      </View>
    );
